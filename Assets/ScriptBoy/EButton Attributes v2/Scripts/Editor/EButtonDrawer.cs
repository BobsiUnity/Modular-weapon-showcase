...
{
    public class EButtonDrawer
    {
        private static bool s_ReverseAttributesOrder;
        [EButton, EButton.BeginHorizontal, InitializeOnLoadMethod]
        public static void CheckAttributesOrder()
        {
            var method = typeof(EButtonDrawer).GetMethod("CheckAttributesOrder");
            var attributes = method.GetCustomAttributes(false);
            s_ReverseAttributesOrder = attributes[0].GetType() != typeof(EButtonAttribute);
        }

        private UIAction[] m_Actions;

        public EButtonDrawer(object target)
        {
            BindingFlags flags =
            ...
            for (int i = 0; i < methods.Length; i++)
            {
                ...
                for (int j = 0; j < attributeCount; j++)
                {
                    ...
                    if (attributeType == typeof(EButtonAttribute))
                    {
                        var eButtonAttribute = (EButtonAttribute)attribute;
                    ...
                    else if (attributeType == typeof(EButton.BeginHorizontalAttribute))
                    {
                        var scopeAttribute = (EButton.BeginHorizontalAttribute)attribute;
                        actionList.Add(new BeginHorizontalScope(scopeAttribute.text));
                        scopeTypeStack.Push(ScopeType.Horizontal);
                    }
                    else if (attributeType == typeof(EButton.EndHorizontalAttribute))
                    {
                        actionList.Add(new EndHorizontalScope());
                        scopeTypeStack.Pop();
                    }
                    else if (attributeType == typeof(EButton.BeginVerticalAttribute))
                    {
                        var scopeAttribute = (EButton.BeginVerticalAttribute)attribute;
                        actionList.Add(new BeginVerticalScope(scopeAttribute.text));
                        scopeTypeStack.Push(ScopeType.Vertical);
                    }
                    else if (attributeType == typeof(EButton.EndVerticalAttribute))
                    {
